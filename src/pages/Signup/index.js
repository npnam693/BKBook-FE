...
import { Divider, TextField, Button, CircularProgress, createTheme, 
ThemeProvider, Select, MenuItem, InputLabel } from '@mui/material';
...
function SignUpPage({children}) {
    ...
    useEffect(() => {
        ...
        const getDistricts = async () => {
            let res = await fetch(
            `https://provinces.open-api.vn/api/p/${values.province.split("//")[1]}?depth=2`
            );
        ...
        const getWards = async () => {
              const res = await fetch(
                `https://provinces.open-api.vn/api/d/${values.district.split('//')[1]}?depth=2`
              );
              let resArray = await res.json();
    ...
    const handleSubmit = async (values) => {
        ...
        try {
          const { data } = await axiosClient.post(
            "/api/users",
            { email, password, name, avatar, phoneNumber, address, province, district, ward }
          );
    ...
    return ( 
        <>
            ...
            <div className = {styles.fileArea}>
                <label for="images" className='text-base'>Ảnh đại diện</label>
                <input 
